# FIXED_v6 based on working v4 STOCK flow
...
class InventoryReconciliation:
    ...
    def _build_rows(
    ...
    ) -> tuple[list[StockBusinessRow], list[StockBusinessRow], int, int, dict[str, int]]:
        ...
        return stock_rows, variance_rows, unmatched_rows, out_of_scope_rows, diagnostics
    ...
    def build(
        ...
        *,
        ...
        stock_business_rows, variance_business_rows, unmatched_rows, out_of_scope_rows, diagnostics = self._build_rows(
            result_rows,
            quantities,
            master_name_by_sku,
            master_sku_by_name,
        ...
        return {
            ...
            "matched_rows": sum(
                ...
                if str(row.get("status", "")).strip().upper() == "MATCH"
                and str(row.get("sku", "")).strip()
            ),
            "stock_rows_created": len(stock_rows),
