...
class Matcher:
    ...
    def _canonical_volume(self, value: str | None) -> tuple[str, float] | None:
        if value is None:
            return None

        text = str(value).strip().upper().replace(",", ".")
        text = " ".join(text.split())
        match = re.search(r"(\d+(?:\.\d+)?)\s*(МЛ|ML|Л|L|ГР|Г|KG|КГ)\b", text)
        if not match:
            return None
    ...
    def _is_svo_144_conditioner(self, item: ArrivalItem) -> bool:
        return (
            self._normalize_value(getattr(item, "category", None)) == "КОНДИЦИОНЕР"
            and self._normalize_value(getattr(item, "brand", None)) == "SVO"
            and self._canonical_volume(getattr(item, "volume", None)) == ("L", 1.44)
        )

    def _volume_matches(self, arrival: ArrivalItem, master: MasterItem) -> bool:
        arrival_volume = self._canonical_volume(getattr(arrival, "volume", None))
        master_volume = self._canonical_volume(getattr(master, "volume", None))

        if arrival_volume is not None and master_volume is not None:
            return (
                arrival_volume[0] == master_volume[0]
                and abs(arrival_volume[1] - master_volume[1]) < 0.0001
            )

        if arrival_volume is not None and master_volume is None:
            return False

        return True

    def _hard_candidate_allowed(self, arrival: ArrivalItem, master: MasterItem) -> bool:
        if not self._product_type_matches(arrival, master):
            return False
        if not self._volume_matches(arrival, master):
            return False
        return True

    def _product_type_matches(self, arrival: ArrivalItem, master: MasterItem) -> bool:
        arrival_product_type = self._normalize_value(effective_product_type(arrival))
        master_product_type = self._normalize_value(effective_product_type(master))
        ...
        # SVO 2.7L perfumed laundry gel products historically exist in
        # MASTER under the conditioner group. Keep this exception narrow.
        arrival_text = self._normalize_value(
            getattr(arrival, "source_name", "")
        )
        if (
            "SVO" in arrival_text
            and ("2,7" in arrival_text or "2.7" in arrival_text)
            and "ГЕЛЬ ДЛЯ СТИРКИ" in arrival_text
            and {
        ...
        for group in compatibility_groups:
            if arrival_product_type in group and master_product_type in group:
                return True
    ...
    def _score_components(self, arrival: ArrivalItem, master: MasterItem) -> dict[str, float]:
        if not self._hard_candidate_allowed(arrival, master):
            return {
                "ProductType": 0.0,
                "Brand": 0.0,
                "Volume": 0.0,
        ...
        if self._volume_matches(arrival, master) and arrival.volume and master.volume:
            volume_score = 40.0
    ...
    def _metadata_matches(self, arrival: ArrivalItem, master: MasterItem) -> bool:
        ...
        for field_name in ("category", "brand"):
    ...
    def _confirmed_review_alias(self, item: ArrivalItem) -> MasterItem | None:
        """Resolve source-specific aliases only when the MASTER SKU is unambiguous."""
        source = str(getattr(item, "source_name", "") or "").strip()
        compact = re.sub(r"\s+", " ", source).strip().upper()
        # Canonicalize GILAR spelling before source-specific rules.
        compact = re.sub(r"GILA[’'`´]R", "GILAR", compact, flags=re.IGNORECASE)
        compact = re.sub(r"GILA\s+R", "GILAR", compact, flags=re.IGNORECASE)

        # SALES: SVO 1 L conditioner aliases confirmed by MASTER.
        sales_rules = (
            # Only aliases confirmed by the current MASTER are allowed to auto-match.
            (r"ГЕЛЬ КОНДИЦИОНЕР.*BABY.*1\s*Л\b", "SKU-025"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*ГОРНЫЙ БРИЗ.*ГОЛУБОЙ.*1\s*Л\b", "SKU-026"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*ЛАВАНДА.*ФИОЛЕТОВЫЙ.*1\s*Л\b", "SKU-027"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*ЛИМОН.*ЖЕЛТЫЙ.*1\s*Л\b", "SKU-028"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*ПИОН.*РОЗОВЫЙ.*1\s*Л\b", "SKU-029"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*РОЗА.*КРАСНЫЙ.*1\s*Л\b", "SKU-030"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*ЧЕРН.*1\s*Л\b", "SKU-031"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*BLACK.*1\s*L\b", "SKU-031"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*FLORAL.*1\s*Л\b", "SKU-028"),
            (r"ГЕЛЬ КОНДИЦИОНЕР.*FLORIAL.*1\s*Л\b", "SKU-028"),
        )
        for pattern, sku in sales_rules:
            if re.search(pattern, compact, re.IGNORECASE):
                matches = [candidate for candidate in self.master_items if candidate.sku == sku]
                if len(matches) == 1:
                    return matches[0]

        # SALES: SVO Elegant 1 L aliases.
        elegant_1l_rules = (
            (r"ЖИДКОЕ МОЮЩЕЕ СРЕДСТВО.*SVO\s+ELEGANT.*БЕЛОГО БЕЛЬЯ.*1\s*Л\b", "SKU-074"),
            (r"ЖИДКОЕ МОЮЩЕЕ СРЕДСТВО.*SVO\s+ELEGANT.*FLORAL.*1\s*Л\b", "SKU-076"),
            (r"ЖИДКОЕ МОЮЩЕЕ СРЕДСТВО.*SVO\s+ELEGANT.*ЧЕРНОГО БЕЛЬЯ.*1\s*Л\b", "SKU-081"),
            (r"ЖИДКОЕ МОЮЩЕЕ СРЕДСТВО.*SVO\s+.*ДЕТСКОГО БЕЛЬЯ.*1\s*Л\b", "SKU-082"),
        )
        for pattern, sku in elegant_1l_rules:
            if re.search(pattern, compact, re.IGNORECASE):
                matches = [candidate for candidate in self.master_items if candidate.sku == sku]
                if len(matches) == 1:
                    return matches[0]

        # SALES: SVO liquid soap 500 ml — exact MASTER SKU-113.
        if re.search(
            r"ЖИДКОЕ МЫЛО\s+500\s*МЛ\s+SVO.*(?:ОЛИВКОВ|OLIVE)",
            compact,
            re.IGNORECASE,
        ):
            matches = [candidate for candidate in self.master_items if candidate.sku == "SKU-113"]
            if len(matches) == 1:
                return matches[0]

        # SALES: GILAR liquid soap 500 ml is represented by MASTER's
        # canonical "Крем-мыло" positions.
        gilar_soap_rules = (
            (r"GILAR\s+МЫЛО\s+ЖИДКОЕ\s+500\s*МЛ.*КОКОСОВ\w*\s+МОЛОК\w*", "SKU-118"),
            (r"ЖИДКОЕ МЫЛО\s+500\s*МЛ\s+GILAR.*COCONUT", "SKU-118"),
            (r"ЖИДКОЕ МЫЛО\s+500\s*МЛ\s+GILAR.*OATMEAL", "SKU-119"),
            (r"ЖИДКОЕ МЫЛО\s+500\s*МЛ\s+GILAR.*ORIENTAL", "SKU-120"),
            (r"ЖИДКОЕ МЫЛО\s+500\s*МЛ\s+GILAR.*PINE\s+FOREST", "SKU-121"),
            (r"ЖИДКОЕ МЫЛО\s+500\s*МЛ\s+GILAR.*(?:CHERRY|СHERRY)", "SKU-122"),
        )
        for pattern, sku in gilar_soap_rules:
            if re.search(pattern, compact, re.IGNORECASE):
                matches = [candidate for candidate in self.master_items if candidate.sku == sku]
                if len(matches) == 1:
                    return matches[0]

        # SALES: Pearlis 4 x 85 g aliases.
        pearlis_rules = (
            (r"PEARLIS.*ГРЕЙПФРУТ.*4\s*ШТ.*85\s*ГР", "SKU-132"),
            (r"PEARLIS.*ДУШИСТЫЙ БУКЕТ.*4\s*ШТ.*85\s*ГР", "SKU-133"),
            (r"PEARLIS.*ЗЕЛЕНЫЙ ЧАЙ.*4\s*ШТ.*85\s*ГР", "SKU-134"),
            (r"PEARLIS.*МОРСКИЕ МИНЕРАЛЫ.*4\s*ШТ.*85\s*ГР", "SKU-135"),
            (r"PEARLIS.*ДУШИСТЫЙ БУКЕТ.*4\s*ШТ.*100\s*ГР", "SKU-136"),
            (r"PEARLIS.*МОРСКИЕ МИНЕРАЛЫ.*4\s*ШТ.*100\s*ГР", "SKU-137"),
            (r"PEARLIS.*РОЗА.*4\s*ШТ.*100\s*ГР", "SKU-138"),
        )
        for pattern, sku in pearlis_rules:
            if re.search(pattern, compact, re.IGNORECASE):
                matches = [candidate for candidate in self.master_items if candidate.sku == sku]
                if len(matches) == 1:
                    return matches[0]

        # SALES: floor cleaner colors.
        floor_rules = (
            (r"МОЮЩЕЕ СРЕДСТВО ДЛЯ МЫТЬЯ ПОЛА.*1\s*Л.*РОЗОВ", "SKU-139"),
            (r"МОЮЩЕЕ СРЕДСТВО ДЛЯ МЫТЬЯ ПОЛА.*1\s*Л.*ФИОЛЕТОВ", "SKU-141"),
        )
        for pattern, sku in floor_rules:
            if re.search(pattern, compact, re.IGNORECASE):
                matches = [candidate for candidate in self.master_items if candidate.sku == sku]
                if len(matches) == 1:
                    return matches[0]

        # SALES: dishwashing orange 750 ml.
        if re.search(r"МОЮЩЕЕ СРЕДСТВО ДЛЯ МЫТЬЯ ПОСУДЫ.*750\s*МЛ.*АПЕЛЬСИН", compact, re.IGNORECASE):
            matches = [candidate for candidate in self.master_items if candidate.sku == "SKU-256"]
            if len(matches) == 1:
                return matches[0]

        # SALES: black 2.7 L conditioner with the agreed floral description.
        if re.search(
            r"ПАРФЮМИРОВАННЫЙ ГЕЛЬ ДЛЯ СТИРКИ.*SVO.*ЧЕРНЫЙ.*2,7\s*Л.*АРОМАТ ЦВЕТОВ,?\s*СВЕЖЕСТИ",
            compact,
            re.IGNORECASE,
        ):
            matches = [candidate for candidate in self.master_items if candidate.sku == "SKU-063"]
            if len(matches) == 1:
                return matches[0]

        # SALES: GILAR 400 ml shampoo aliases.  These are explicit MASTER SKUs.
        gilar_shampoo_rules = (
        ...
        # SALES: SVO 1.44 L conditioner aliases confirmed by MASTER.
        svo_144_aliases = {
            "MAGINA": "SKU-033",
            "VELVET": "SKU-034",
            "LOTOS": "SKU-035",
            ...
            "PAPATYA": "SKU-040",
            "FLORAL MIST": "SKU-041",
            "FLORIAL MIST": "SKU-041",
            "PASSION FRUIT": "SKU-042",
            "ORCHIDE": "SKU-043",
            "ORHIDEYA": "SKU-043",
            "SAFFRAN": "SKU-044",
            "SHAFRAN": "SKU-044",
            ...
            "SHALK": "SKU-045",
            "JASMIN": "SKU-046",
            "ROMANTIK ROSE": "SKU-047",
            "ROMANTIK": "SKU-047",
            "SWEET TROPIC": "SKU-048",
            "SWEET TROPIK": "SKU-048",
            "ROSE": "SKU-049",
        }
        if self._is_svo_144_conditioner(item):
            sku = svo_144_aliases.get(compact)
            if sku is not None:
                matches = [candidate for candidate in self.master_items if candidate.sku == sku]
                if len(matches) == 1:
                    return matches[0]
        ...
        source_aliases = {
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р–РЃР›РўР«Р™"): "PAPATYA",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р–Р•Р›РўР«Р™"): "PAPATYA",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "РљР РђРЎРќР«Р™"): "PION",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р РћР—РћР’Р«Р™"): "ROSE",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "РЎРќР™"): "MIDNIGHT",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р¤РћР›Р•РўРћР’Р«Р™"): "LAVENDER",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р§РЃР РќР«Р™"): "BLACK",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р§Р•Р РќР«Р™"): "BLACK",
            ("РљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "РђР РћРњРђРў РЎРўР РђРЎРўРљРћРќР”Р¦РћРќР•Р ", "SVO", "2,7 Р›", "Р РћРњРђРќРўР§Р•РЎРљР• Р¦Р’Р•РўР«"): "ROMANTIK ROSE",
            ("РџРћР РћРЁРћРљ РЎРўР РђР›Р¬РќР«Р™", "SVO", "5 РљР“", "РЁР•Р™РҐ"): "MAGINA",
            ("РџРћР РћРЁРћРљ РЎРўР РђР›Р¬РќР«Р™", "SVO", "6 РљР“", "РЎР’Р•Р–Р•РЎРўР¬ Р“РћР "): "MOUNTAIN BREEZE",
            ("РџРћР РћРЁРћРљ РЎРўР РђР›Р¬РќР«Р™", "SVO", "9 РљР“", "РЎР’Р•Р–Р•РЎРўР¬ Р“РћР "): "MOUNTAIN BREEZE",
            ("РћРўР‘Р•Р›Р’РђРўР•Р›Р¬", "SVO", "750 Р“", "OXYGEN Р”Р›РЇ Р‘Р•Р›Р«РҐ"): "WHITE",
            ("РћРўР‘Р•Р›Р’РђРўР•Р›Р¬", "SVO", "750 Р“", "OXYGEN Р”Р›РЇ Р¦Р’Р•РўРќР«РҐ"): "COLOR",
            ("РџРћРЎРЈР”Рђ РњРћР®Р©Р•Р• РЎР -Р’Рћ", "SVO", "750 Р“", "РђРџР•Р›Р¬РЎРќ ORANGE"): "ORANGE",
    ...
    def _find_exact_master(self, item: ArrivalItem) -> MasterItem | None:
        ...
        for field_name in ("product_type", "category", "brand", "aroma"):
    ...
    def _collect_candidates(self, item: ArrivalItem) -> list[MasterItem]:
        ...
        def hard_filter(group: list[MasterItem]) -> list[MasterItem]:
            return [candidate for candidate in group if self._hard_candidate_allowed(item, candidate)]
        ...
        category_candidates = hard_filter(self._lookup_candidates(item.category))
        brand_candidates = hard_filter(self._lookup_candidates(item.brand))
        volume_candidates = hard_filter(self._lookup_candidates(item.volume))
        ...
        for candidate in self.master_items:
            if not self._hard_candidate_allowed(item, candidate):
                continue
    ...
    def _is_exact_triple_match(self, item: ArrivalItem, master: MasterItem) -> bool:
        ...
        arrival_volume = self._canonical_volume(item.volume)
        master_volume = self._canonical_volume(master.volume)
    ...
    def _confirmed_price_alias(self, item: ArrivalItem) -> MasterItem | None:
        ...
        rules = (
            ...
            ("OXGEEN БЕЛЫЙ", "SKU-143"),
            ("OXGEEN ЦВЕТНОЙ", "SKU-144"),
            ("OXYGEN БЕЛЫЙ", "SKU-143"),
            ("OXYGEN ЦВЕТНОЙ", "SKU-144"),
            ...
            ("GILAR ШАМПУНЬ 400 МЛ INTENSIV", "SKU-005"),
            ("GILAR ШАМПУНЬ 400 МЛ LOVELY", "SKU-007"),
            ("GILAR ШАМПУНЬ 600 МЛ KERATIN ЗЕЛ ЧАЙ", "SKU-017"),
        )
    ...
    def match(self, item: ArrivalItem) -> ArrivalItem:
        ...
        # Hard source-family guard for SALES rows with no reliable MASTER metadata.
        # These names must never be fuzzy-matched into unrelated household categories.
        source_upper = re.sub(
            r"\s+",
            " ",
            str(getattr(item, "source_name", "") or ""),
        ).strip().upper()
        source_upper = re.sub(r"GILA[’'`´]R", "GILAR", source_upper)
        source_upper = re.sub(r"GILA\s+R", "GILAR", source_upper)

        pearlis_green_tea_85 = (
            "PEARLIS" in source_upper
            and "ЗЕЛЕНЫЙ ЧАЙ" in source_upper
            and re.search(r"4\s*ШТ", source_upper, re.IGNORECASE)
            and re.search(r"85\s*(?:ГР|Г)", source_upper, re.IGNORECASE)
        )
        gilar_keratin_green_tea_600 = (
            "GILAR" in source_upper
            and "ШАМПУНЬ" in source_upper
            and re.search(r"600\s*МЛ", source_upper, re.IGNORECASE)
            and "KERATIN" in source_upper
            and "ЗЕЛ ЧАЙ" in source_upper
        )

        blocked_family = None
        if re.search(r"\bЭНЕРГЕТИК\b", source_upper):
            blocked_family = "ЭНЕРГЕТИК"
        elif (
            re.search(r"ЧАЙЛЕМОН|\bЧАЙ\b", source_upper)
            and not pearlis_green_tea_85
            and not gilar_keratin_green_tea_600
        ):
            blocked_family = "ЧАЙ"

        if blocked_family is not None:
            item.status = "REVIEW"
            item.review_reasons = ["UNSUPPORTED_SOURCE_FAMILY"]
            item.review_explanation = {
                "confidence": 0.0,
                "reasons": item.review_reasons,
                "source_family": blocked_family,
                "candidates": [],
            }
            return item

        if self._is_svo_144_conditioner(item) and source_upper == "FRUT":
            item.status = "REVIEW"
            item.review_reasons = ["UNCONFIRMED_VARIANT"]
            item.review_explanation = {
                "confidence": 0.0,
                "reasons": item.review_reasons,
                "candidates": [],
        ...
        confirmed_alias = self._confirmed_price_alias(item) or self._confirmed_review_alias(item)

        if confirmed_alias is not None:
            master = confirmed_alias
        else:
        ...
        if master is None:
            ...
            if not candidates:
                source_text = self._source_text(item)
                diagnostic_candidates: list[MasterItem] = []
                for candidate in self.master_items:
                    if self._product_type_matches(item, candidate):
                        continue
                    if not self._volume_matches(item, candidate):
                        continue

                    item_brand = getattr(item, "brand", None)
                    candidate_brand = getattr(candidate, "brand", None)
                    if item_brand and candidate_brand:
                        if self._normalize_value(item_brand) != self._normalize_value(candidate_brand):
                            continue

                    if self._source_master_similarity(source_text, candidate) < 0.18:
                        continue
                ...
                if diagnostic_candidates:
                    ...
                    item.confidence = 0.0
                    item.candidates = [
                        entry["candidate"] for entry in scored_candidates
                    ]
                    item.review_reasons = self._build_review_reasons(
                        scored_candidates
                    )
                    item.review_explanation = {
                        "confidence": item.confidence,
                        "reasons": item.review_reasons,
                        "candidates": self._candidate_explanations(
                            scored_candidates
                        ),
                        "best_candidate_rejected_reason": item.review_reasons,
                    }
                    item.status = "REVIEW"
                    return item
