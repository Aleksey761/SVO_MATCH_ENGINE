...
def _find_sales_columns(ws) -> tuple[int | None, int | None]:
    ...
    # Legacy SALES layout: SKU + column N ("ВСЕГО").
...
def _load_inventory_stock_by_sku_master(
    inventory_file: Path | None,
    *,
    master_by_sku: dict[str, object],
    correction_layer: NameCorrectionLayer,
    alias_by_inventory_name: dict[str, str] | None = None,
) -> tuple[dict[str, int], int, int]:
    """Load physical stock from inventory using deterministic MASTER-driven mapping.

    Priority:
      1) approved exact InventoryName -> SKU alias;
      2) exact MASTER_NAME after the existing INVENTORY name-correction layer.
    No fuzzy matching and no SKU invention.
    Returns (stock_by_sku, mapped_rows, ambiguous/unresolved_rows).
    """
    if inventory_file is None or not inventory_file.exists():
        return {}, 0, 0

    aliases = alias_by_inventory_name or {}
    stock_by_sku: dict[str, int] = {}
    mapped_rows = 0
    unresolved_rows = 0
    ...
    for row_idx in range(3, ws.max_row + 1):
        raw_name = str(ws.cell(row=row_idx, column=3).value or "").strip()
        if not raw_name:
            continue

        sku = str(aliases.get(raw_name, "") or "").strip()
        if not sku:
            decision = correction_layer.apply_to_name(raw_name, source="INVENTORY")
            corrected_name = str(decision.after_name or "").strip()
            resolved = correction_layer.resolve_master_exact(corrected_name)
            if resolved is not None:
                sku = str(resolved[0] or "").strip()
    ...
    return stock_by_sku, mapped_rows, unresolved_rows
...
def _execute_finance_pipeline(args: argparse.Namespace) -> dict:
    ...
    inventory_alias_map_file = Path("output") / "INVENTORY_ALIAS_MAP.xlsx"

    # MASTER is the driving table. PRICE/SALES/INVENTORY only enrich MASTER SKU rows.
    ...
    if not master_by_sku:
        raise ValueError(f"MASTER contains no usable SKU rows: {master_file}")

    correction_layer = NameCorrectionLayer(master_items)

    price_rows = [
        row for row in _load_price_match_rows(price_match_file)
        if row["Status"] == "MATCH"
    ]
    price_by_sku: dict[str, dict[str, object]] = {}
    duplicate_price_skus: set[str] = set()
    for row in price_rows:
        sku = str(row["SKU"] or "").strip()
        if not sku:
            continue
        if sku in price_by_sku:
            duplicate_price_skus.add(sku)
        price_by_sku[sku] = row
    ...
    alias_by_inventory_name = _load_inventory_alias_map(inventory_alias_map_file)
    alias_stock_by_sku, alias_applied_rows = _load_inventory_stock_by_sku_exact_alias(
        inventory_file,
        alias_by_inventory_name=alias_by_inventory_name,
    )

    # Deterministic MASTER-driven fallback for stock when the alias map is absent/incomplete.
    master_stock_by_sku, master_stock_rows, inventory_unresolved_rows = _load_inventory_stock_by_sku_master(
        inventory_file,
        master_by_sku=master_by_sku,
        correction_layer=correction_layer,
        alias_by_inventory_name=alias_by_inventory_name,
    )
    stock_by_sku = dict(master_stock_by_sku)
    for sku, qty in alias_stock_by_sku.items():
    ...
    missing_master_for_price = 0
    missing_price = 0
    for sku, master_item in master_by_sku.items():
        price_row = price_by_sku.get(sku)
        if price_row is None:
            missing_price += 1

        master_name = str(master_item.master_name or "").strip()
        unit_cost = _to_decimal_or_none(price_row["UnitCost"]) if price_row else None
        retail_price = _to_decimal_or_none(price_row["RetailPrice"]) if price_row else None
        ...
        if unit_cost is not None and retail_price is not None:
            ...
            gross_profit = calculator.calculate_gross_profit(
                revenue=revenue,
                cogs=cogs,
            )
            margin_pct = calculator.calculate_gross_margin_percent(
                revenue=revenue,
    ...
    # Detect PRICE rows that claim MATCH but do not belong to MASTER.
    for sku in price_by_sku:
        if sku not in master_by_sku:
            missing_master_for_price += 1
    ...
    complete = sum(
        ...
        if row["HasSales"] and row["HasCost"] and row["HasStock"]
    )
    ...
    ws_lineage.append([
        ...
        str(master_file),
        "MASTER",
        "MASTER is the driving table; one finance row per MASTER SKU",
    ])
    ws_lineage.append([
        "MASTER_NAME",
        str(master_file),
        "MASTER",
        "Canonical MASTER_NAME; PRICE never overrides MASTER nomenclature",
    ])
    ws_lineage.append([
        "SalesQty",
        str(sales_file) if sales_file else "N/A",
        "SALES_MATCH",
        "SKU -> ВСЕГО (with supported SALES quantity header aliases), else 0",
    ])
    ws_lineage.append([
        "RetailPrice",
        str(price_match_file),
        "PRICE_MATCH",
        "MATCH price by SKU; missing price remains empty",
    ])
    ...
    ws_lineage.append([
        ...
        str(price_match_file),
        "PRICE_MATCH",
        "MATCH cost by SKU; missing cost remains empty",
    ])
    ...
    ws_lineage.append([
        ...
        "computed",
        "FinanceCalculator",
        "(gross_profit / revenue) * 100; revenue=0 => 0",
    ])
    ws_lineage.append([
        "StockQty",
        str(inventory_file) if inventory_file else "N/A",
        "INVENTORY/MASTER",
        "Inventory column 15 mapped deterministically to MASTER SKU; approved aliases have priority",
    ])
    ws_lineage.append(["StockValue", "computed", "FinanceCalculator", "stock_qty * unit_cost"])
    ws_lineage.append([
        "Отгружено",
        str(inventory_file) if inventory_file else "N/A",
        "InventoryInputReader",
        "supplier_return - sales, SKU-first mapping with deterministic fallback",
    ])
    ...
    ws_lineage.append(["MASTER SKU without PRICE", missing_price])
    ws_lineage.append(["PRICE MATCH SKU outside MASTER", missing_master_for_price])
    ws_lineage.append(["Duplicate PRICE MATCH SKU", len(duplicate_price_skus)])
    ws_lineage.append(["Ambiguous inventory fallback rows", ambiguous_fallback_rows])
    ws_lineage.append(["Inventory alias rows applied", alias_applied_rows])
    ws_lineage.append(["Inventory MASTER-mapped rows", master_stock_rows])
    ws_lineage.append(["Inventory unresolved rows", inventory_unresolved_rows])
    ...
    print(f"[finance] MASTER SKU without PRICE: {missing_price}")
    print(f"[finance] Inventory MASTER-mapped rows: {master_stock_rows}")
    print(f"[finance] Inventory unresolved rows: {inventory_unresolved_rows}")
    print(f"[finance] Alias rows: {alias_applied_rows}")
    if inventory_parse_warning:
    ...
    return {
        ...
        "missing_master_for_price": missing_master_for_price,
        "duplicate_price_skus": len(duplicate_price_skus),
        ...
        "inventory_alias_rows_applied": alias_applied_rows,
        "inventory_master_mapped_rows": master_stock_rows,
        "inventory_unresolved_rows": inventory_unresolved_rows,
    }
