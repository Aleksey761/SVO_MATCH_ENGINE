...
class InventoryReconciliation:
    ...
    def _resolve_inventory_by_normalizer(
        self,
        source_name: object,
        master_records: list[object],
    ) -> str | None:
        """Resolve inventory name through existing normalizer and MASTER fields."""
        try:
            item = ArrivalItem(
                row_number=0,
        ...
        for master in master_records:
            sku = str(getattr(master, "sku", "") or "").strip()
            if not sku:
                continue

            checks = [
                getattr(normalized, "brand", None),
                getattr(normalized, "variant", None),
                getattr(normalized, "volume", None),
                getattr(normalized, "aroma", None),
            ]

            master_text = " ".join(
                str(getattr(master, "master_name", "") or "").upper().split()
            )

            score = sum(
                1 for value in checks
                if value and str(value).upper() in master_text
            )
        ...
        for quantity in quantities:
            ...
            if explicit_sku:
                ...
                if not candidate_skus:
                    resolved_sku = self._resolve_inventory_by_normalizer(
                        quantity.get("source_name", ""),
                        master_records,
                    )
                    if resolved_sku:
    ...
    def build(
        ...
        *,
        ...
        master_records = self.loader.load_master(master_file)
        master_name_by_sku: dict[str, str] = {}
        for master_item in master_records:
            sku = str(master_item.sku or "").strip()
            if not sku:
                continue
            master_name_by_sku[sku] = str(master_item.master_name or "").strip() or (
