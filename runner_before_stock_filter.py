...
def _load_inventory_stock_by_master_name_exact(
    inventory_file: Path | None,
    *,
    master_by_sku: dict[str, object],
) -> tuple[dict[str, int], int]:
    """Read legacy REVISION stock strictly by exact MASTER_NAME/source alias.

    This is a finance-mode fallback when STOCK_MATCH.xlsx is not supplied.
    It intentionally uses the established REVISION layout (name in column C,
    physical stock in column O) and performs exact normalized-name matching only.
    """
    if inventory_file is None or not inventory_file.exists():
        return {}, 0

    master_name_to_skus: dict[str, set[str]] = {}
    for sku, master_item in master_by_sku.items():
        master_name = " ".join(
            str(getattr(master_item, "master_name", "") or "").strip().upper().split()
        )
        if master_name:
            master_name_to_skus.setdefault(master_name, set()).add(sku)

    if not master_name_to_skus:
        return {}, 0
    ...
    for row_idx in range(3, ws.max_row + 1):
        ...
        if not name:
            continue
        candidate_skus = master_name_to_skus.get(name)
        if not candidate_skus or len(candidate_skus) != 1:
            continue
        sku = next(iter(candidate_skus))
        stock_qty = _to_int_or_zero(ws.cell(row=row_idx, column=15).value)
        stock_by_sku[sku] = stock_by_sku.get(sku, 0) + stock_qty

    return stock_by_sku, 0
...
def _execute_finance_pipeline(args: argparse.Namespace) -> dict:
    ...
    alias_map_file = output_file.parent / "INVENTORY_ALIAS_MAP.xlsx"
    alias_by_inventory_name = _load_inventory_alias_map(alias_map_file)

    inventory_alias_rows_applied = 0
    stock_source = "PRICE_MATCH_FALLBACK+REVISION_EXACT_MASTER_NAME"
    if stock_match_arg is not None:
        stock_by_sku = _load_stock_match_by_sku(stock_match_file)
        stock_source = "STOCK_MATCH"
    else:
        # Direct finance-mode calls must not consume an unrelated pre-existing
        # output/STOCK_MATCH.xlsx. Retain matched PRICE stock as a conservative
        # per-SKU fallback, then overlay exact REVISION links and approved aliases.
        stock_by_sku = {}
        for price_row in price_rows:
            sku = str(price_row.get("SKU") or "").strip()
            if not sku:
                continue
            stock_by_sku[sku] = _to_int_or_zero(price_row.get("StockQty"))

        exact_stock_by_sku, _ = _load_inventory_stock_by_master_name_exact(
            inventory_file,
            master_by_sku=master_by_sku,
        )
        for sku, qty in exact_stock_by_sku.items():
            stock_by_sku[sku] = qty

        if alias_by_inventory_name:
            alias_stock_by_sku, inventory_alias_rows_applied = _load_inventory_stock_by_sku_exact_alias(
                inventory_file,
            ...
            for sku, qty in alias_stock_by_sku.items():
                stock_by_sku[sku] = qty
            stock_source = (
                "PRICE_MATCH_FALLBACK+INVENTORY_ALIAS_MAP+REVISION_EXACT_MASTER_NAME"
            )
    ...
    ws_lineage.append([
        "StockQty",
        str(stock_match_file) if stock_source == "STOCK_MATCH" else str(inventory_file),
        stock_source,
        (
            "Fact stock by SKU from STOCK_MATCH.xlsx (STOCK_RESULT: SKU + Фактический остаток)"
            if stock_source == "STOCK_MATCH"
            else (
                "Exact approved INVENTORY_ALIAS_MAP.xlsx mapping to REVISION column O"
                if stock_source == "INVENTORY_ALIAS_MAP"
                else "Exact normalized MASTER_NAME match to REVISION name column C; stock from column O"
            )
        ),
    ])
    ...
    ws_lineage.append(["Stock rows loaded", len(stock_by_sku)])
    ws_lineage.append(["Inventory alias rows applied", inventory_alias_rows_applied])
    ...
    print(f"[finance] Stock source: {stock_source}")
    print(f"[finance] Alias rows  : {inventory_alias_rows_applied}")
    if inventory_parse_warning:
    ...
    return {
        ...
        "inventory_alias_rows_applied": inventory_alias_rows_applied,
    }
