...
class SalesLoader(ArrivalLoader):
    ...
    def load(self, filename: str | Path) -> list[ArrivalItem]:
        ...
        for excel_row, row in enumerate(
            ...
            # Keep the dataclass default (match_name=None) for ordinary rows.
            # Contextual rewrite is only an internal matching aid and should not
            # change equality/serialized source semantics for unchanged names.
            if match_name != text:
