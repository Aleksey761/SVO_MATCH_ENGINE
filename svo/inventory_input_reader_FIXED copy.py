...
class InventoryInputReader:
    ...
    def read_inventory_quantities(self, inventory_file: str | Path) -> list[dict[str, int]]:
        ...
        # Inventory revisions may contain only the current dated stock snapshot.
        # In that valid format there is no separate "opening" snapshot, so the
        # actual dated stock column is sufficient for reading the inventory.
        if product_col is None or actual_col is None:
            raise ValueError(
                "Required inventory columns not found: НАИМЕНОВАНИЕ / "
                "Остаток на складе (шт) with date header"
            )

        # Keep the legacy payload contract: when there is no distinct opening
        # snapshot, use the actual stock snapshot as opening as well.
        if opening_col is None:
            opening_col = actual_col
