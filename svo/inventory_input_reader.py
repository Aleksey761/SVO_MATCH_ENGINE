...
class InventoryInputReader:
    ...
    def _find_inventory_header_row(self, ws) -> int:
        ...
        for row in range(1, max_scan_rows + 1):
            ...
            has_opening = any("ОСТАТОК НА СКЛАДЕ" in value and "20.02.2024" in value for value in normalized)
    ...
    def read_inventory_quantities(self, inventory_file: str | Path) -> list[dict[str, int]]:
        ...
        for idx, normalized in enumerate(normalized_headers, start=1):
            if "ОСТАТОК НА СКЛАДЕ" in normalized and "20.02.2024" in normalized:
                opening_col = idx
                break
        ...
        if product_col is None or opening_col is None:
            raise ValueError("Required REVISION columns not found: НАИМЕНОВАНИЕ / Остаток на складе (шт) на 20.02.2024")
