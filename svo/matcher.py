    ...
    def _score(self, arrival: ArrivalItem, master: MasterItem) -> float:
        ...
        if arrival_aroma and master_aroma:
            ...
            if normalized_arrival_aroma == normalized_master_aroma:
                ...
                score += 15.0 * self._aroma_similarity(arrival_aroma, master_aroma)

        # Use raw source text as an extra differentiator between close variant candidates.
        ...
        if source_variant_similarity > 0.0:
            score += 8.0 * source_variant_similarity
        ...
        if master is None:
            ...
            if scored_candidates:
                ...
                close_candidates = [
                    ...
                    if score >= best_score - (2.0 if (item.aroma or item.variant) else 5.0)
                ]
                ...
                if len(close_candidates) > 1:
                    if len(close_candidates) == 2:
                        ...
                        if best_score >= 93.0 and score_gap >= 0.5:
                ...
                if best_score >= 100.0:
                    ...
                    score_gap = best_score - second_score
                    if (
                        len(close_candidates) == 1
                        and best_score >= 60.0
                        and (len(scored_candidates) == 1 or score_gap >= 3.0)
                    ):
                        master = best_candidate
                    elif best_score >= 82.0 and score_gap >= 8.0:
                        master = best_candidate
                    elif len(scored_candidates) == 1 and best_score >= 75.0:
                        master = best_candidate
            else:
                best_score = 0.0
