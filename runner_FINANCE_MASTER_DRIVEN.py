...
def _execute_finance_pipeline(args: argparse.Namespace) -> dict:
    ...
    # Finance is MASTER-driven: every canonical MASTER SKU must remain in the
    # finance dataset. PRICE_MATCH only supplies optional price/cost fields.
    price_match_rows = _load_price_match_rows(price_match_file)
    price_by_sku = {
        str(row["SKU"] or "").strip(): row
        for row in price_match_rows
        if str(row["SKU"] or "").strip() and row["Status"] == "MATCH"
    }
    ...
    for master_item in master_items:
        ...
        if not price_row:
            # SKU exists in MASTER but has no PRICE MATCH row; keep it in Finance
            # with missing price/cost and zero fallback stock.
            price_row = {}

        master_name = str(master_item.master_name or "").strip()
        unit_cost = _to_decimal_or_none(price_row.get("UnitCost"))
        retail_price = _to_decimal_or_none(price_row.get("RetailPrice"))
        stock_qty = int(alias_stock_by_sku.get(sku, _to_int_or_zero(price_row.get("StockQty"))))
    ...
    ws_lineage.append(["SKU", str(master_file), "MASTER", "Canonical SKU list; Finance is MASTER-driven"])
    ws_lineage.append(["MASTER_NAME", str(master_file), "MASTER", "Canonical MASTER_NAME from Loader master"])
    ws_lineage.append(["SalesQty", str(sales_file) if sales_file else "N/A", "SALES_MATCH", "Column ВСЕГО by SKU when available, else 0"])
    ws_lineage.append(["RetailPrice", str(price_match_file), "PRICE_MATCH", "RetailPrice for MATCHed SKU; blank when no PRICE MATCH"])
    ws_lineage.append(["Revenue", "computed", "FinanceCalculator", "sales_qty * retail_price"])
    ws_lineage.append(["UnitCost", str(price_match_file), "PRICE_MATCH", "UnitCost for MATCHed SKU; blank when no PRICE MATCH"])
    ...
    ws_lineage.append([
        "StockQty",
        str(inventory_alias_map_file) if alias_by_inventory_name else str(price_match_file),
        "INVENTORY_ALIAS_MAP/PRICE_MATCH",
        "Exact InventoryName alias map to SKU uses inventory column 15 when alias exists; otherwise StockQty from matched PRICE",
    ])
