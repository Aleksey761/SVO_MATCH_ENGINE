...
class InventoryReconciliation:
    ...
    def _resolve_inventory_sku_from_master(
        self,
        source_name: object,
        master_records: list[object],
    ) -> str | None:
        """Resolve inventory rows that do not carry SKU using MASTER names."""
        value = " ".join(str(source_name or "").strip().upper().split())
        if not value:
            return None
        ...
        for item in master_records:
            ...
            master_name = " ".join(
                str(getattr(item, "master_name", "") or "").strip().upper().split()
            )
            if sku and master_name and (
                value == master_name
                or value in master_name
                or master_name in value
            ):
