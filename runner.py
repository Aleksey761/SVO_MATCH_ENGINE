...
def _build_final_report(
    *,
    ...
    ws_dashboard.append(["STOCK Out of scope", int(summary_payload.get("stock_out_of_scope", 0))])
...
def _write_run_summary(output_path: Path, payload: dict[str, object]) -> None:
    text = "\n".join(
        [
            ...
            f"Out of scope : {payload.get('stock_out_of_scope', 0)}",
            "",
            "Price changes",
...
def _write_business_metrics_xlsx(output_path: Path, *, summary_payload: dict[str, object]) -> None:
    ...
    ws.append(["STOCK out of scope", int(summary_payload.get("stock_out_of_scope", 0))])
    ws.append(["Duration sec", int(summary_payload.get("duration_sec", 0))])
    output_path.parent.mkdir(parents=True, exist_ok=True)
    wb.save(output_path)


def _count_out_of_scope(result_file: Path) -> int:
    if not result_file.exists():
        return 0
    try:
        wb = load_workbook(filename=result_file, data_only=True)
        ws = wb.active
        headers = {_normalize_header(ws.cell(row=1, column=c).value): c for c in range(1, ws.max_column + 1)}
        status_col = headers.get("STATUS")
        if status_col is None:
            return 0
        return sum(
            1 for row in range(2, ws.max_row + 1)
            if str(ws.cell(row=row, column=status_col).value or "").strip().upper() == "OUT_OF_SCOPE"
        )
    except Exception:
        return 0
...
def _run_full(engine: Engine, args: argparse.Namespace) -> dict:
    ...
    if inventory_file is not None and inventory_file.exists() and sales_items:
        try:
            ...
            stock_out_of_scope = _count_out_of_scope(stock_source_result_file)
        except Exception as exc:
            errors.append(
                {
                    "Module": "STOCK",
    ...
    summary_payload = {
        ...
        "stock_out_of_scope": stock_out_of_scope,
        "price_inc": price_inc,
        "price_dec": price_dec,
