...
class Normalizer:
    ...
    def normalize(self, item: ArrivalItem) -> ArrivalItem:
        ...
        # Resolve SVO laundry terminology conflicts:
        # "гель кондиционер" = conditioner
        # "парфюмированный гель для стирки" = laundry detergent (ЖМС)
        item.product_type, item.category = self._fix_svo_laundry_type(
            text,
            item.product_type,
            item.category,
        )
    ...
    def _fix_svo_laundry_type(
        ...
        if "гель кондиционер" in lower or "гель-кондиционер" in lower:
            return "Кондиционер", "Кондиционер"

        if (
            "парфюмированный гель для стирки" in lower
            or "гель для стирки" in lower
            or "гель д/стирки" in lower
        ):
            return "ЖМС", "ЖМС"
